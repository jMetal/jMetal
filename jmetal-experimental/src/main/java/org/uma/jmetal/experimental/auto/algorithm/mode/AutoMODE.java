...
/**
 * Class to configure MODE with an argument string using class {@link EvolutionaryAlgorithm}
 *
...
public class AutoMODE {
  ...
  private void variation(String[] args) {
    DifferentialEvolutionCrossoverParameter crossover = new DifferentialEvolutionCrossoverParameter(args,
            List.of("RAND_1_BIN",
            "RAND_1_EXP",
            "RAND_2_EXP",
            "BEST_1_BIN",
  ...
  /**
   * Creates an instance of MODE from the parsed parameters
   *
   * @return
   */
  EvolutionaryAlgorithm<DoubleSolution> create() {
    ...
    Ranking<DoubleSolution> ranking = new FastNonDominatedSortRanking<>(new DefaultDominanceComparator<>());
    ...
    Replacement<DoubleSolution> replacement =
            new RankingAndDensityEstimatorReplacement<>(preferenceForReplacement, RankingAndDensityEstimatorReplacement.RemovalPolicy.oneShot);

    Termination termination =
        new TerminationByEvaluations(maximumNumberOfEvaluationsParameter.getValue());

    var mode = new EvolutionaryAlgorithm<>(
            "NSGA-II",
            evaluation,
