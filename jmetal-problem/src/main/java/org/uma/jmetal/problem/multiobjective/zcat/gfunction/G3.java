...
public class G3 implements Function<double[], double[]> {
  private final int numberOfVariables;
  private final int paretoSetDimension;

  public G3(int numberOfVariables, int paretoSetDimension) {
  ...
  public double[] apply(double[] y) {
    ...
    for (int j = 1; j <= numberOfVariables - paretoSetDimension; ++j) {
      ...
      for (int i = 1; i <= paretoSetDimension; ++i) {
        sum += Math.pow(Math.cos(Math.PI * y[i - 1] + AngleCalculator.calculateTheta(j, paretoSetDimension, numberOfVariables)), 2.0);
      }
