...
public class MaF07 extends AbstractDoubleProblem {
  ...
  public @NotNull DoubleSolution evaluate(@NotNull DoubleSolution solution) {
    ...
    double g, h;
    var result = 0.0;
      for (var idx = numberOfObjectives - 1; idx < numberOfVariables; idx++) {
        var v2 = x[idx];
          result += v2;
      }
      g = result;
    ...
    var sub1 = 1 + g;
    var sum = 0.0;
    var bound = numberOfObjectives - 1;
      for (var i1 = 0; i1 < bound; i1++) {
        var v1 = (x[i1] * (1 + Math.sin(3 * Math.PI * x[i1])) / sub1);
          sum += v1;
      }
      h = sum;
