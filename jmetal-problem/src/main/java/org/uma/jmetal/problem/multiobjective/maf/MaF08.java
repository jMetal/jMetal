...
public class MaF08 extends AbstractDoubleProblem {
  ...
  public @NotNull DoubleSolution evaluate(@NotNull DoubleSolution solution) {
    ...
    var count = 0;
      for (var i1 = 0; i1 < numberOfObjectives; i1++) {
        var sqrt = Math.sqrt(Math.pow(const8[i1][0] - x[0], 2) + Math.pow(const8[i1][1] - x[1], 2));
  ...
  public static double[][] polygonpoints(int m, double r) {
    var startp = new double[2];
    startp[0] = 0;
    startp[1] = 1;
    var p1 = new double[m][2];
    p1[0] = startp;
    ...
    for (var i = 1; i < m; i++) {
      ...
      for (var i = 0; i < m; i++) {
        var doubles = p1[m - i - 1];
          list.add(doubles);
      }
    var p = list.toArray(new double[0][]);
    return p;
  }
