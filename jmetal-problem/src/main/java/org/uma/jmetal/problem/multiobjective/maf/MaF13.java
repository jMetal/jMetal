  ...
  public DoubleSolution evaluate(DoubleSolution solution) {
    ...
    var count = 0;
      for (var i1 = 0; i1 < numberOfVariables_; i1++) {
        var v = x[i1] - 2 * x[1] * Math.sin(2 * Math.PI * x[0] + (i1 + 1) * Math.PI / numberOfVariables_);
    ...
    var J3 = new ArrayList<Integer>();
    @NotNull ArrayList<Integer> J4 = new ArrayList<Integer>();
    double sub1 = 0, sub2 = 0, sub3 = 0, sub4 = 0;
    for (var i = 4; i <= numberOfVariables_; i = i + 3) {
      J1.add(i);
      sub1 += Math.pow(y[i - 1], 2);
    }
    sub1 = 2 * sub1 / J1.size();
    for (var i = 5; i <= numberOfVariables_; i = i + 3) {
      J2.add(i);
      sub2 += Math.pow(y[i - 1], 2);
    }
    sub2 = 2 * sub2 / J2.size();
    for (var i = 3; i <= numberOfVariables_; i = i + 3) {
      J3.add(i);
      sub3 += Math.pow(y[i - 1], 2);
    }
    sub3 = 2 * sub3 / J3.size();
    for (var i = 4; i <= numberOfVariables_; i++) {
      J4.add(i);
    ...
    for (var i = 3; i < numberOfObjectives_; i++) {
