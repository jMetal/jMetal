 ...
 */
public class F13 implements Function<double[], double[]> {
  int numberOfObjectives ;

  public F13(int numberOfObjectives) {
  ...
  public double[] apply(double[] y) {
    ...
    F[0] = 1.0 - F[0] / (numberOfObjectives - 1.0);
    ...
    for (int j = 2; j <= numberOfObjectives - 1; ++j) {
      ...
      F[j - 1] += Math.cos(y[numberOfObjectives - j + 1 - 1] * Math.PI / 2);
      F[j - 1] = 1.0 - F[j - 1] / (numberOfObjectives - j + 1);
      assert (0 <= F[j - 1] && F[j - 1] <= 1.0);
    }

    F[numberOfObjectives - 1] = 1.0 - (Math.cos((2 * k - 1) * y[0] * Math.PI) + 2 * y[0] + 4 * k * (1 - y[0]) - 1) / (4.0 * k);
