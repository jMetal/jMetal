 ...
 */
public class F3 implements Function<double[], double[]> {
  int numberOfObjectives ;

  public F3(int numberOfObjectives) {
  ...
  public double[] apply(double[] y) {
    ...
    for (int j = 2; j <= numberOfObjectives - 1; ++j) {
      ...
      F[j - 1] += (1 - y[numberOfObjectives - j + 1 - 1]);
      F[j - 1] = F[j - 1] / (numberOfObjectives - j + 1);
      assert (0 <= F[j - 1] && F[j - 1] <= 1.0);
    }

    F[numberOfObjectives - 1] = 1 - y[0];
