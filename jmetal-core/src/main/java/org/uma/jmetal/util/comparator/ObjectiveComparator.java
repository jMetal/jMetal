...
public class ObjectiveComparator<S extends Solution<?>> implements Comparator<S>, Serializable {
  ...
  public int compare(S solution1, S solution2) {
    ...
    if (solution1 == null) {
      ...
      result =  -1;
    } else if (solution1.objectives().size() <= objectiveId) {
      throw new JMetalException("The solution1 has " + solution1.objectives().size()+ " objectives "
          + "and the objective to sort is " + objectiveId) ;
    } else if (solution2.objectives().size() <= objectiveId) {
      throw new JMetalException("The solution2 has " + solution2.objectives().size()+ " objectives "
          + "and the objective to sort is " + objectiveId) ;
    } else {
      double objective1 = solution1.objectives().get(this.objectiveId);
      double objective2 = solution2.objectives().get(this.objectiveId);
