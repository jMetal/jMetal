...
import org.uma.jmetal.util.experiment.util.MultithreadedExperimentExecutorJ8;
 ...
 */
public class ExecuteAlgorithmsJ8<S extends Solution<?>, Result> implements ExperimentComponent {
  private Experiment<S, Result> experiment;

  /** Constructor */
  public ExecuteAlgorithmsJ8(Experiment<S, Result> configuration) {
  ...
  public void run() {
    ...
    MultithreadedExperimentExecutorJ8<S, Result> parallelExecutor ;

    parallelExecutor = new MultithreadedExperimentExecutorJ8<S, Result>(experiment.getNumberOfCores()) ;
    parallelExecutor.start(this);

    experiment.getAlgorithmList()
            .stream()
            .forEach(alg -> parallelExecutor.addTask(new Object[]{alg, alg.getRunId(), experiment}));
