 ...
 * This operator constructs offspring by preserving adjacency (edge) information from the parent permutations.
 * It builds an edge map (also known as an adjacency matrix) of adjacent genes from both parents and constructs offspring by traversing this map,
 * selecting edges that exist in either parent to maintain as much parental adjacency as possible.
...
public class EdgeRecombinationCrossover implements CrossoverOperator<PermutationSolution<Integer>> {
    ...
    private List<Integer> edgeRecombination(List<Integer> parent1, List<Integer> parent2) {
        ...
        for (int i = 0; i < parent1.size(); i++) {
            ...
            edgeMap.get(value).add(parent1.get((i - 1 + parent1.size()) % parent1.size())); // Previous element (wrap around)
        }
        ...
        for (int i = 0; i < parent2.size(); i++) {
            ...
            edgeMap.get(value).add(parent2.get((i - 1 + parent2.size()) % parent2.size())); // Previous element (wrap around)
        }
