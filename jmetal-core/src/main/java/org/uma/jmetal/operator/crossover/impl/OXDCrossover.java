 ...
 * This operator combines elements of the standard Order Crossover (OX) and handles duplicates to ensure valid permutations.
 * It selects a subsequence from one parent and fills the remaining positions from the other parent,
 * maintaining the order and avoiding duplicate genes to produce valid offspring permutations.
...
public class OXDCrossover implements CrossoverOperator<PermutationSolution<Integer>> {
    ...
    private void repairOffspring(PermutationSolution<Integer> offspring, PermutationSolution<Integer> parent, PermutationSolution<Integer> otherParent, int cuttingPoint1, int cuttingPoint2) {
        ...
        // Create a set of genes that are already in the offspring segment (between cuttingPoint1 and cuttingPoint2)
        Set<Integer> genesInSegment = new HashSet<>(offspring.variables().subList(cuttingPoint1, cuttingPoint2 + 1));

        // Start from the position after cuttingPoint2 and continue wrapping around until cuttingPoint1 is reached
