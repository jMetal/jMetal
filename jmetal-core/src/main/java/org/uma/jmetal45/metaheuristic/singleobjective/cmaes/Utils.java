...
package org.uma.jmetal45.metaheuristic.singleobjective.cmaes;
      ...
      for (j = 0; j < N; ++j) {
        ...
        if (Math.abs(cc - C[i > j ? i : j][i > j ? j : i]) / Math.sqrt(C[i][i] * C[j][j]) > 1e-10
          ...
          JMetalLogger.logger.log(Level.SEVERE,
            "org.uma.jmetal45.metaheuristic.cmaes.Utils.checkEigenSystem(): WARNING - imprecise experimentoutput detected "
              + s
          );
          ++res;
        }
        if (Math.abs(dd - (i == j ? 1 : 0)) > 1e-10) {
          s = i + " " + j + " " + dd;
          JMetalLogger.logger.log(Level.SEVERE,
            "org.uma.jmetal45.metaheuristic.cmaes.Utils.checkEigenSystem(): WARNING - imprecise experimentoutput detected (Q not orthog.) "
              + s);
          ++res;
        }
      }
