...
/**
 * Tournament-based replacement strategy that selects N solutions from the combined parent and
 * offspring populations using tournament selection, where N is the size of the parent population.
 *
 * <p>This strategy provides configurable selection pressure through the tournament size parameter,
 * making it useful for automatic algorithm configuration.
 *
 * <p>For multi-objective optimization, a natural choice for the comparator is one based on Pareto
 * ranking and density estimation (e.g., crowding distance), which can be obtained from {@link
 * org.uma.jmetal.component.util.RankingAndDensityEstimatorPreference#getComparator()}.
 *
 * <p>Control parameters:
 *
 * <ul>
 *   <li><b>tournamentSize</b>: Number of solutions competing in each tournament. Range: [2,
 *       populationSize + offspringSize]. Default: 2 (binary tournament). Higher values increase
...
public class TournamentReplacement<S extends Solution<?>> implements Replacement<S> {
   ...
   * populations using tournament selection.
   *
