...
public class AutoConfigurationExperimentTable extends HtmlTable<Object> {
    ...
    private final static String[] CREATE_INITIAL_SOLUTIONS = new String[] {"random", "latinHypercubeSampling", "scatterSearch"};
    private final static String[] SELECTION = new String[] {"tournament", "random"};
    private final static String[] CROSSOVER_AND_MUTATION = new String[] {"crossoverAndMutationVariation"};
    private final static String[] CROSSOVER = new String[] {"SBX", "BLX_ALPHA"};
    private final static String[] REPAIR_STRATEGY = new String[] {"random", "round", "bounds"};
    private final static String[] MUTATION = new String[] {"uniform", "polynomial"};
    private final static String[] RESULT = new String[] {"externalArchive", "population"};
    private final static int[] POPULATION_SIZE_ARCHIVE = new int[] {10, 20, 50, 100, 200};
    private final static int[] OFFSPRING_POPULATION = new int[] {1, 10, 50, 100, 200, 400};
            ...
            switch (column.name()) {
                ...
                case "selectionTournamentSize": {
                    ...
                    if ((int) getResultValueOf(table, columnDependsOn) == valueDependsOn) {
                        Table filteredTable = tableFilteredByDependencies(table, columnDependsOn, valueDependsOn);
    ...
    private Table tableFilteredByDependencies(Table table, String columnNameDependsOn, int valueDependsOn) {
        ...
        for (int columnIndex = 0; columnIndex < data[0].length; columnIndex++) {
            ...
            for (int rowIndex = 0; rowIndex < data.length; rowIndex++) {
                if (this.data[rowIndex][columnIndex] != "") {
                    switch (column.name()) {
                        case "createInitialSolutions": {
                            this.data[rowIndex][columnIndex] = CREATE_INITIAL_SOLUTIONS[(int) data[rowIndex][columnIndex]];
                            break;
                        }
                        case "selection": {
                        ...
                        case "variation": {
                            this.data[rowIndex][columnIndex] = CROSSOVER_AND_MUTATION[(int) data[rowIndex][columnIndex]];
                            break;
                        }
                        case "offspringPopulationSize": {
                            this.data[rowIndex][columnIndex] = OFFSPRING_POPULATION[(int) data[rowIndex][columnIndex]];
                            break;
                        }
                        case "crossover": {
                        ...
                        case "crossoverRepairStrategy": {
                            this.data[rowIndex][columnIndex] = REPAIR_STRATEGY[(int) data[rowIndex][columnIndex]];
                            break;
                        }
                        case "mutation": {
                        ...
                        case "populationSizeWithArchive": {
                            this.data[rowIndex][columnIndex] = POPULATION_SIZE_ARCHIVE[(int) data[rowIndex][columnIndex]];
                            break;
                        }
                    }
                }
