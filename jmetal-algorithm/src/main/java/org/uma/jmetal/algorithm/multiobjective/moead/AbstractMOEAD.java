  ...
  double fitnessFunction(S individual, double[] lambda) throws JMetalException {
    ...
    if (MOEAD.FunctionType.TCHE.equals(functionType)) {
      ...
      for (int n = 0; n < problem.getNumberOfObjectives(); n++) {
        double diff = Math.abs(individual.objectives().get(n) - idealPoint.getValue(n));
      ...
      double sum = 0.0;
      for (int n = 0; n < problem.getNumberOfObjectives(); n++) {
        sum += (lambda[n]) * individual.objectives().get(n);
      }
      ...
      for (int i = 0; i < problem.getNumberOfObjectives(); i++) {
        d1 += (individual.objectives().get(i) - idealPoint.getValue(i)) * lambda[i];
      ...
      for (int i = 0; i < problem.getNumberOfObjectives(); i++) {
        d2 += Math.pow((individual.objectives().get(i) - idealPoint.getValue(i)) - d1 * (lambda[i] / nl), 2.0);
      }
      d2 = Math.sqrt(d2);
