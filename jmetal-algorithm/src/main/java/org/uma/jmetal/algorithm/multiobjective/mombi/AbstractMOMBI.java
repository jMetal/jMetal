...
public abstract class AbstractMOMBI<S extends Solution<?>> extends AbstractGeneticAlgorithm<S,List<S>> {
	...
	protected void updateReferencePoint(S s) {
		for (int i = 0; i < s.objectives().size(); i++)
			this.getReferencePoint().set(i, Math.min(this.getReferencePoint().get(i),s.objectives().get(i)));
	}

	protected void updateNadirPoint(S s) {
		for (int i = 0; i < s.objectives().size(); i++)
			this.getNadirPoint().set(i, Math.max(this.getNadirPoint().get(i),s.objectives().get(i)));
	}
