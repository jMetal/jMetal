...
import org.uma.jmetal.algorithm.multiobjective.wasfga.util.*;
...
public class WASFGA<S extends Solution<?>> implements Algorithm<List<S>> {
  ...
  WeightVector.NORMALIZE normalization;
  ...
  public WASFGA(Problem<S> problem, int populationSize, int maxEvaluations,
                ...
                WeightVector.NORMALIZE normalization, boolean estimatePoints, ReferencePoint referencePoint,
                AchievementScalarizingFunction<S> achievementScalarizingFunction) {
    super();
  ...
  public void run() throws JMetalException {
    ...
    this.weights = WeightVector.invertWeights(weights, WeightVector.NORMALIZE.TRUE);
    ...
    while (evaluations < maxEvaluations) {
      ...
      List<S> front;
      //population.clear();
      population = new ArrayList<S>(populationSize);
