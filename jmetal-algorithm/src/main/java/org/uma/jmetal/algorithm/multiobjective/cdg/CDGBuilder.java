...
public class CDGBuilder implements AlgorithmBuilder<AbstractCDG<DoubleSolution>> {
  ...
  public CDGBuilder(Problem<DoubleSolution> problem) {
    ...
    childGridNum_ = (int) Math.pow(childGrid_, problem.numberOfObjectives());
    childGridNum_++;
    subproblemNum_ = (int) Math.pow(k_, problem.numberOfObjectives() - 1);
    subproblemNum_ = subproblemNum_ * problem.numberOfObjectives();
  }
