...
public class DifferentialEvolution2 extends AbstractDifferentialEvolution<Solution> {
   ...
   * @param evaluator
   */
  public DifferentialEvolution2(DoubleProblem problem, int maxEvaluations, int populationSize,
  ...
  @Override protected List<DoubleSolution> reproduction(List<DoubleSolution> matingPopulation) {
    ...
    selectionOperator.setIndex(currentIndividual);
    List<DoubleSolution> parents = selectionOperator.execute(matingPopulation);

    crossoverOperator.setCurrentSolution(matingPopulation.get(currentIndividual));
...
*/
  ...
  @Override protected List<DoubleSolution> replacement(List<DoubleSolution> population,
    ...
    if (comparator.compare(population.get(currentIndividual), offspringPopulation.get(0)) < 0) {
      pop.add(population.get(currentIndividual));
    } else {
