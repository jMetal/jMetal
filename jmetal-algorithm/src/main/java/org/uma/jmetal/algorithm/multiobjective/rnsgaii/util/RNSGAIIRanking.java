...
public class RNSGAIIRanking<S extends Solution<?>> extends GenericSolutionAttribute<S, Integer>
  ...
  public Ranking<S> computeRanking(List<S> population) {
    ...
    for (int i = 0; i < population.get(0).objectives().length; i++) {
      ...
      double objetiveMinn = population.get(0).objectives()[i];
      double objetiveMaxn = population.get(population.size() - 1).objectives()[i];
  ...
  private double preference(
      S solution1, S solution2, List<Double> upperBounds, List<Double> lowerBounds) {
    double result = 0.0D;

    for (int indexOfObjective = 0;
        indexOfObjective < solution1.objectives().length;
        indexOfObjective++) {
      if (upperBounds != null && lowerBounds != null) {
        result =
            result
                + ((Math.abs(
                        solution1.objectives()[indexOfObjective]
                            - solution2.objectives()[indexOfObjective]))
                    / (upperBounds.get(indexOfObjective) - lowerBounds.get(indexOfObjective)));
      } else {
        result =
            result
                + Math.abs(
