...
public class SMPSOMeasuresNovelty extends AbstractParticleSwarmOptimization<DoubleSolution, List<DoubleSolution>> implements Measurable {
   ...
   * Constructor
   */
  public SMPSOMeasuresNovelty(DoubleProblem problem, int swarmSize, BoundedArchive<DoubleSolution> leaders,
    ...
    for (int i = 0; i < swarm.size(); i++) {
      ...
      for (int var = 0; var < particle.getNumberOfVariables(); var++) {
        speed[i][var] = velocityConstriction(constrictionCoefficient(c1, c2) * (
                        inertiaWeight(Math.toIntExact(iterations.get()), maxIterations, wmax, wmin) * speed[i][var] +
  ...
  public int getIterations() {
    return Math.toIntExact(iterations.get());
  }
