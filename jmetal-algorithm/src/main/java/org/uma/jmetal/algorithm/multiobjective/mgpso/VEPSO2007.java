 ...
 */
public class VEPSO2007 implements Algorithm<List<DoubleSolution>> {
  ...
  public VEPSO2007(DoubleProblem problem,
                   int swarmSize,
                   int maxIterations,
  ...
  protected void initializeLeaders(List<List<DoubleSolution>> swarmList) {
    for (int i = 0; i < psoIsland.size(); i++) {
      psoIsland.get(i).initializeGlobalBest(swarmList.get(i)); ;
    }
  }

  protected void initializeParticlesMemory(List<List<DoubleSolution>> swarmList) {
    for (int i = 0; i < psoIsland.size(); i++) {
      psoIsland.get(i).initializeParticleBest(swarmList.get(i)) ;
  ...
  protected void updateLeaders(List<List<DoubleSolution>> swarmList) {
    for (int i = 0; i < psoIsland.size(); i++) {
      psoIsland.get(i).updateGlobalBest(swarmList.get(i)) ;
    }
  }

  protected void updateParticlesMemory(List<List<DoubleSolution>> swarmList) {
    for (int i = 0; i < psoIsland.size(); i++) {
      psoIsland.get(i).updateParticleBest(swarmList.get(i));
    }
  }
        ...
        if (localBest != globalBest) {
          for (int var = 0; var < particle.variables().size(); var++) {
            pso.getSwarmSpeedMatrix()[i][var] = weight * pso.getSwarmSpeedMatrix()[i][var] +
                    r1 * (localBest.variables().get(var) - particle.variables().get(var)) +
                    r2 * (globalBest.variables().get(var) - particle.variables().get(var));
          }
        } else {
          for (int var = 0; var < particle.variables().size(); var++) {
            pso.getSwarmSpeedMatrix()[i][var] = weight * pso.getSwarmSpeedMatrix()[i][var] +
                    r1 * (localBest.variables().get(var) -
  ...
  protected DoubleSolution selectGlobalBest() {
    ...
    Comparator<DoubleSolution> crowdingDistanceComparator = new CrowdingDistanceDensityEstimator<DoubleSolution>().getComparator() ;
  ...
  public String getDescription() {
    return "VEPSO based on standard PSO 2007";
  }
}
