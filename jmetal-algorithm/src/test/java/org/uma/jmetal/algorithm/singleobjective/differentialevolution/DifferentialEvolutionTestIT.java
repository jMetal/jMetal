...
public class DifferentialEvolutionTestIT {
  ...
  public void startup() {
    ...
    algorithm =
        new DifferentialEvolution(
            problem, maxEvaluations, populationSize, crossover, selection, evaluator);
  }
  ...
  public void shouldCreateInitialPopulationWhenPopulationSizeIsBiggerThanZero() {
    ...
    DoubleSolution[] expectedSolution =
        new DoubleSolution[] {
  ...
  public void shouldReproduction() {
    ...
    for (int i = 0; i < populationSize; i++) {
      TestCase.assertEquals(
          String.format("Result %d different from expected.", i), children.get(0), result.get(i));
    }
  }
  ...
  public void shouldReplacemen2t() {
    ...
    List<DoubleSolution> expectedSolutionList = Arrays.<DoubleSolution>asList(solution4, solution1);
