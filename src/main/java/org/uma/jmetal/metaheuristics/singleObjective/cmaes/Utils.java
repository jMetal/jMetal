...
package org.uma.jmetal.metaheuristics.singleObjective.cmaes;
      ...
      for (j = 0; j < N; ++j) {
        ...
        if (Math.abs(cc - C[i > j ? i : j][i > j ? j : i]) / Math.sqrt(C[i][i] * C[j][j]) > 1e-10
          ...
          Configuration.logger_.log(Level.SEVERE,
            "org.uma.jmetal.metaheuristics.cmaes.Utils.checkEigenSystem(): WARNING - imprecise result detected "
              + s
          );
          ++res;
        }
        if (Math.abs(dd - (i == j ? 1 : 0)) > 1e-10) {
          s = i + " " + j + " " + dd;
          Configuration.logger_.log(Level.SEVERE,
            "org.uma.jmetal.metaheuristics.cmaes.Utils.checkEigenSystem(): WARNING - imprecise result detected (Q not orthog.) "
              + s);
          ++res;
        }
      }
