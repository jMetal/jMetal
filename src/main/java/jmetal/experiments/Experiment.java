...
public abstract class Experiment {
  ...
  public String[] algorithmList_; // List of the names of the algorithms to be executed
  ...
  public void runExperiment() throws JMException, IOException {
    ...
    System.out.println("Experiment: Number of algorithms: " + algorithmList_.length);
    ...
    Thread[] p = new RunExperiment[numberOfExecutionThreads_];
    for (int i = 0; i < numberOfExecutionThreads_; i++) {
      //p[i] = new Experiment(map_, i, numberOfThreads, problemList_.length);
      p[i] = new RunExperiment(this, map_, i, numberOfExecutionThreads_, problemList_.length);
      p[i].start();
    }
  ...
  public void initExperiment(String[] args) {
    ...
    setAlgorithmList(algorithmList_);
    ...
    int numberOfAlgorithms = algorithmList_.length;
    ...
    map_.put("algorithmNameList", algorithmList_);
    ...
    map_.put("paretoFrontFile", paretoFrontFileList_);
...
//  public void algorithmSettings(String problemName, int problemId, Algorithm[] algorithm)  {
//    int numberOfAlgorithms = algorithmNameList_.length;
//    System.out.println("Number of algorithms: " + numberOfAlgorithms) ;
//
//    HashMap[] parameters = new HashMap[numberOfAlgorithms];
//
//    for (int i = 0; i < numberOfAlgorithms; i++) {
//      parameters[i] = new HashMap();
//      if (!(paretoFrontFileList_[problemIndex] == null) && !paretoFrontFileList_[problemIndex].equals("")) {
...
//      try {
//        settings = new SettingsFactory().getSettingsObject(algorithmNameList_[i], settingsParams);
...
//  } ;
  ...
  private void checkParetoFronts() {
    if (paretoFrontDirectory_.equals("") || paretoFrontDirectory_ == null) {
      ...
      for (int i = 0; i < problemList_.length; i++) {
        if (paretoFrontFileList_[i] == null || paretoFrontFileList_[i].equals("")) {
          paretoFrontFileList_[i] = "";
          generateReferenceFronts(i);
        } else {
          String filePath = paretoFrontDirectory_ + "/" + paretoFrontFileList_[i];
          File f = new File(filePath);
          if (!f.exists()) {
            paretoFrontFileList_[i] = "";
            generateReferenceFronts(i);
          } else {
            frontPath_[i] = paretoFrontDirectory_ + "/" + paretoFrontFileList_[i];
          } // if
        } // if
      } // for
    } // if
  ...
  public void generateQualityIndicators() {
    ...
    if (indicatorList_.length > 0) {
      ...
      for (int algorithmIndex = 0; algorithmIndex < algorithmList_.length; algorithmIndex++) {

        String algorithmDirectory;
        algorithmDirectory = experimentBaseDirectory_ + "/data/" + algorithmList_[algorithmIndex] + "/";
  ...
  public void generateReferenceFronts(int problemIndex) {
    ...
    for (String anAlgorithmNameList_ : algorithmList_) {
  ...
  public void generateLatexTables() throws FileNotFoundException, IOException {
    ...
    for (int indicator = 0; indicator < indicatorList_.length; indicator++) {
      ...
      for (int problem = 0; problem < problemList_.length; problem++) {
        data[indicator][problem] = new Vector[algorithmList_.length];

        for (int algorithm = 0; algorithm < algorithmList_.length; algorithm++) {
          data[indicator][problem][algorithm] = new Vector();

          String directory = experimentBaseDirectory_;
          directory += "/data/";
          directory += "/" + algorithmList_[algorithm];
    ...
    for (int indicator = 0; indicator < indicatorList_.length; indicator++) {
      ...
      for (int problem = 0; problem < problemList_.length; problem++) {
        mean[indicator][problem] = new double[algorithmList_.length];
        median[indicator][problem] = new double[algorithmList_.length];
        stdDeviation[indicator][problem] = new double[algorithmList_.length];
        iqr[indicator][problem] = new double[algorithmList_.length];
        min[indicator][problem] = new double[algorithmList_.length];
        max[indicator][problem] = new double[algorithmList_.length];
        numberOfValues[indicator][problem] = new int[algorithmList_.length];
  ...
  void printMeanStdDev(String fileName, int indicator, double[][][] mean, double[][][] stdDev) throws IOException {
    ...
    os.write("\\hline");
    // write table head
    for (int i = -1; i < algorithmList_.length; i++) {
      if (i == -1) {
        os.write(" & ");
      } else if (i == (algorithmList_.length - 1)) {
        os.write(" " + algorithmList_[i] + "\\\\" + "\n");
      } else {
        os.write("" + algorithmList_[i] + " & ");
      }
    }
    ...
    for (int i = 0; i < problemList_.length; i++) {
      ...
      if ((Boolean) indicatorMinimize_.get(indicatorList_[indicator])) {// minimize by default
        ...
        for (int j = 0; j < (algorithmList_.length); j++) {
      ...
      for (int j = 0; j < (algorithmList_.length - 1); j++) {
        if (j == bestIndex) {
      ...
      if (bestIndex == (algorithmList_.length - 1)) {
        os.write("\\cellcolor{gray95}");
      }
      m = String.format(Locale.ENGLISH, "%10.2e", mean[indicator][i][algorithmList_.length - 1]);
      s = String.format(Locale.ENGLISH, "%8.1e", stdDev[indicator][i][algorithmList_.length - 1]);
      os.write("$" + m + "_{" + s + "}$ \\\\" + "\n");
    } // for
  ...
  void printMedianIQR(String fileName, int indicator, double[][][] median, double[][][] IQR) throws IOException {
    ...
    for (int i = 0; i < problemList_.length; i++) {
      ...
      m = String.format(Locale.ENGLISH, "%10.2e", median[indicator][i][algorithmList_.length - 1]);
      s = String.format(Locale.ENGLISH, "%8.1e", IQR[indicator][i][algorithmList_.length - 1]);
      os.write("$" + m + "_{" + s + "}$ \\\\" + "\n");
    } // for
    //os.write("" + mean[0][problemList_.length-1][algorithmNameList_.length-1] + "\\\\"+ "\n" ) ;
  ...
  public synchronized int[] getNextProblem() {
    ...
    if (this.algorithmList_.length == 1) {
      alg = 0;
    }
    ...
    if (this.problemIndex < this.problemList_.length - 1) {
      ...
      if (this.algorithmIndex < this.algorithmList_.length - 1) {
        this.algorithmIndex++;
        this.problemIndex = 0;
      } else {
  ...
  public void setAlgorithmList(String[] values) {
    if (configuration_.getProperty("algorithmList") == null) {
      algorithmList_ = values ;
    }
    else {
      algorithmList_ = configuration_.getProperty("algorithmNameList").split(",") ;
    }
  }
