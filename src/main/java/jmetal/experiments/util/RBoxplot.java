...
public class RBoxplot {
  ...
  public static  void generateScripts(int rows,
    ...
    for (int indicator = 0; indicator <  experiment.indicatorList_.length; indicator++) {
      ...
      for (int i = 0; i <  experiment.algorithmList_.length; i++) {
        os.write("file" +  experiment.algorithmList_[i] +
                "<-paste(resultDirectory, \"" +
                 experiment.algorithmList_[i] + "\", sep=\"/\")" + "\n");
        os.write("file" +  experiment.algorithmList_[i] +
                "<-paste(file" +  experiment.algorithmList_[i] + ", " +
                "problem, sep=\"/\")" + "\n");
        os.write("file" +  experiment.algorithmList_[i] +
                "<-paste(file" +  experiment.algorithmList_[i] + ", " +
                "indicator, sep=\"/\")" + "\n");
        os.write( experiment.algorithmList_[i] + "<-scan(" + "file" +  experiment.algorithmList_[i] + ")" + "\n");
        os.write("\n");
      } // for

      os.write("algs<-c(");
      for (int i = 0; i <  experiment.algorithmList_.length - 1; i++) {
        os.write("\"" +  experiment.algorithmList_[i] + "\",");
      } // for
      os.write("\"" +  experiment.algorithmList_[ experiment.algorithmList_.length - 1] + "\")" + "\n");
